        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 border border-border-subtle text-content-tertiary hover:bg-hover rounded-xl transition-colors"
            >
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <h3 className="text-content font-medium">{caseItem.title}</h3>
                    <p className="text-content-secondary text-sm">{caseItem.plaintiff} vs {caseItem.defendant}</p>
                  </div>
                  <div className="flex items-center gap-6">
                    {caseItem.claim_amount && (
                      <div className="text-right">
                        <p className="text-content-secondary text-xs">Claim Amount</p>
                    ...
                    <div className="text-right">
                      <p className="text-content-secondary text-xs">Opened</p>
                      <p className="text-content-tertiary">{caseItem.opened_at}</p>
                    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <h3 className="text-xl font-semibold text-content">{caseItem.title}</h3>
        </div>
        <div className="flex gap-2">
          <StatusBadge status={caseItem.status} />
          <StatusBadge status={caseItem.type} variant="outline" />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-input rounded-lg p-4">
          <p className="text-content-secondary text-sm">Plaintiff</p>
          <p className="text-content font-medium">{caseItem.plaintiff}</p>
        </div>
        <div className="bg-input rounded-lg p-4">
          <p className="text-content-secondary text-sm">Defendant</p>
          <p className="text-content font-medium">{caseItem.defendant}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {caseItem.court_name && (
          <div className="bg-input rounded-lg p-4">
            <p className="text-content-secondary text-sm">Court</p>
            <p className="text-content">{caseItem.court_name}</p>
          </div>
        )}
        <div className="bg-input rounded-lg p-4">
          <p className="text-content-secondary text-sm">Jurisdiction</p>
          <p className="text-content">{caseItem.jurisdiction || '-'}</p>
        </div>
        {caseItem.claim_amount && (
          <div className="bg-input rounded-lg p-4">
            <p className="text-content-secondary text-sm">Claim Amount</p>
            <p className="text-content font-medium">
        ...
        {caseItem.external_counsel_name && (
          <div className="bg-input rounded-lg p-4">
            <p className="text-content-secondary text-sm">External Counsel</p>
            <p className="text-content">{caseItem.external_counsel_name}</p>
          </div>
        )}
      </div>
      ...
      {/* Subject Matter */}
      <div className="bg-input rounded-lg p-4">
        <p className="text-content-secondary text-sm mb-2">Subject Matter</p>
        <p className="text-content">{caseItem.subject_matter}</p>
      </div>

      {/* Timeline */}
      {(caseItem.timeline?.length ?? 0) > 0 && (
        <div>
          <h4 className="text-content font-medium mb-3 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {caseItem.timeline.map((event) => (
              <div key={event.id} className="flex items-start gap-4">
                ...
                <div className="flex-1 bg-input rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-content font-medium">{event.title}</span>
                    <span className="text-content-secondary text-sm">{event.date}</span>
                  </div>
                  {event.description && <p className="text-content-tertiary text-sm">{event.description}</p>}
                  {event.location && <p className="text-content-secondary text-xs mt-1">{event.location}</p>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {caseItem.documents.map((doc) => (
              <div key={doc.id} className="bg-input rounded-lg p-3 flex items-center gap-3">
                <FileText size={18} className="text-amber-400" />
                <div>
                  <p className="text-content text-sm">{doc.name}</p>
                  <p className="text-content-secondary text-xs">{doc.document_type}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-input">
                <tr>
                  <th className="text-left p-3 text-sm font-medium text-content-secondary">Description</th>
                  <th className="text-left p-3 text-sm font-medium text-content-secondary">Category</th>
                  <th className="text-left p-3 text-sm font-medium text-content-secondary">Amount</th>
                  <th className="text-left p-3 text-sm font-medium text-content-secondary">Status</th>
                </tr>
              </thead>
              <tbody>
                {caseItem.costs.map((cost) => (
                  <tr key={cost.id} className="border-t border-border">
                    <td className="p-3 text-content">{cost.description}</td>
                    <td className="p-3"><StatusBadge status={(cost.category ?? '').replace('_', ' ')} variant="outline" size="sm" /></td>
                    <td className="p-3 text-content">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: cost.currency }).format(cost.amount)}</td>
      ...
      {caseItem.outcome && (
        <div className="bg-emerald-900/30 border border-emerald-600/50 rounded-lg p-4">
          ...
          <p className="text-content">{caseItem.outcome}</p>
  ...
  return (
    <div className="space-y-4">
      {counsels.map((counsel) => (
        <div key={counsel.id} className="bg-input rounded-lg p-4">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h4 className="text-content font-medium">{counsel.firm_name}</h4>
              <p className="text-content-secondary text-sm">{counsel.contact_person}</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              ...
              <p className="text-content">{counsel.email}</p>
            </div>
            <div>
              <p className="text-content-secondary">Phone</p>
              <p className="text-content">{counsel.phone}</p>
            </div>
            <div>
              <p className="text-content-secondary">Cases Handled</p>
              <p className="text-content">{counsel.cases_handled} ({counsel.cases_won} won)</p>
            </div>
            <div>
              <p className="text-content-secondary">Rating</p>
              <p className="text-amber-400">{'★'.repeat(Math.round(counsel.performance_rating))}</p>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {counsel.specialization.map((spec) => (
              <span key={spec} className="px-2 py-1 bg-hover rounded text-xs text-content-tertiary">{spec}</span>
            ))}
          </div>
        </div>
      ))}
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          <label className="block text-sm font-medium text-content-tertiary mb-1">Case Title</label>
          <input
            type="text"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-content-tertiary mb-1">Priority</label>
          <select
        ...
        <div>
          <label className="block text-sm font-medium text-content-tertiary mb-1">Plaintiff</label>
          <input
            type="text"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-content-tertiary mb-1">Defendant</label>
          <input
            type="text"
            required
        ...
        <div className="col-span-2">
          <label className="block text-sm font-medium text-content-tertiary mb-1">Subject Matter</label>
          <textarea
            required
        ...
        <div>
          <label className="block text-sm font-medium text-content-tertiary mb-1">Claim Amount</label>
          <input
            type="number"
